...
LPDIRECT3DTEXTURE9			g_pTexBomb = NULL;
LPDIRECT3DVERTEXBUFFER9		g_pVB_2 = NULL;
LPDIRECT3DINDEXBUFFER9		g_pIB_2 = NULL;
D3DXMATRIXA16				g_matBomb;
...
HRESULT InitVB()
{
	...
	{
		{ 15.0f , 5.0f , 0.5f , 1.0f , 0xffff0000} ,
		...
		//{ 350.0f , 50.0f , 0.5f , 1.0f , 0xff00ffff }
	};

	if (FAILED(g_pd3dDevice->CreateVertexBuffer(6 * sizeof(CUSTOMVERTEX), 0, D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &g_pVB_2, NULL)))
		return E_FAIL;

	void* pVertices_2;
	if (FAILED(g_pVB_2->Lock(0, sizeof(vertices), (void**)&pVertices_2, 0)))
		return E_FAIL;
	memcpy(pVertices_2, vertices, sizeof(vertices));
	g_pVB_2->Unlock();
...
HRESULT InitIB()
{
	...
	if (FAILED(g_pd3dDevice->CreateIndexBuffer(4 * sizeof(MYINDEX), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &g_pIB_2, NULL)))
		return E_FAIL;

	VOID* pIndeces;
	if (FAILED(g_pIB_2->Lock(0, sizeof(indices), (void**)&pIndeces, 0)))
		return E_FAIL;
...
VOID Animate()
{
	...
	D3DXMatrixTranslation(&g_matBomb, v.x, v.y, v.z);
	//D3DXMatrixIdentity(&g_matBomb);
...
void CleanUp()
{
	...
	SAFE_RELEASE(g_pIB);
	SAFE_RELEASE(g_pVB);
	SAFE_RELEASE(g_pIB_2);
	SAFE_RELEASE(g_pVB_2);
...
void Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		D3DXMatrixRotatioY()
		//D3DXMATRIXA16 temp = *(ZCamera::GetInstance()->GetBillMatrix()) * g_matBomb;

		
		g_pd3dDevice->SetTransform(D3DTS_WORLD, &g_matBomb);
		//g_pd3dDevice->SetTransform(D3DTS_WORLD, &temp);

		g_pd3dDevice->SetStreamSource(0, g_pVB_2, 0, sizeof(CUSTOMVERTEX2));
		g_pd3dDevice->SetFVF(D3DFVF_CUSTOMVVERTEX);
		g_pd3dDevice->SetIndices(g_pIB_2);
		g_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 2);
		//g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0,2,0,2);
