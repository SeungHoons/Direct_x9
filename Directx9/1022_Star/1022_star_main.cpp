...
void SetupCamera()
{
	...
	D3DXVECTOR3 vEyePt(0.0f, 10.0f, -40.0f);
...
VOID Animate()
{
	...
	D3DXMatrixRotationY(&g_matRParent, t);
	t += 0.05f;
	//D3DXMatrixRotationZ(&g_matRChild, GetTickCount() / 500.0f);
	//D3DXMatrixTranslation(&g_matTMChild, 3, 0, 3);
}
...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		g_Sun.init(g_pd3dDevice, g_pVB, g_pIB);

		matWorld = g_matRParent * g_matTMParent;
		//DrawMesh(&matWorld);
		g_Sun.render(&g_matTMParent);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		//g_Sun = new Sun();
		g_Sun.setAngle(0.01f);
		g_Sun.init(g_pd3dDevice, g_pVB, g_pIB);
		g_Sun.setPosition(D3DXVECTOR3(0, 0, 0));
		tempSun = new Sun();
		tempSun->setAngle(0.1f);
		tempSun->setPosition(D3DXVECTOR3(3, 0, 3));
		g_Sun.setChiled(*tempSun,10);
